...
def run(months: int, seed: Optional[int]) -> None:
    ...
    print(
        f"President: {us.president.party.value}; House: {us.congress.house_control.value}; Senate: {us.congress.senate_control.value}"
    )
    print(
        f"Growth: {us.growth:.3%}; Unemp: {us.unemployment:.1f}%; Inflation: {us.inflation:.1f}%"
    )
    print(
        f"Approval - President: {us.opinion.approval_president:.1f}, Congress: {us.opinion.approval_congress:.1f}"
    )
    if us.log:
        print("Recent log:")
